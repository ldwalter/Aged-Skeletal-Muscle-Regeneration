...
ggarrange(plot_list[[14]], plot_list[[15]], plot_list[[16]], 
          plot_list[[18]] + labs(title = "Monocytes/Macrophages \n(Patrolling; Ctsa+)"), 
          plot_list[[4]], plot_list[[22]], plot_list[[27]], plot_list[[1]],  
          common.legend = T, legend = "right", ncol = 4, nrow =2, align = "hv")

# Plot the cell types that do not have a statistically significant difference in their cell type dynamics
ggarrange(plot_list[[2]], plot_list[[3]], plot_list[[5]], plot_list[[6]], plot_list[[7]], 
          plot_list[[8]], plot_list[[9]], plot_list[[10]], plot_list[[11]], plot_list[[12]],
          plot_list[[13]], plot_list[[17]], plot_list[[19]], plot_list[[20]], plot_list[[21]],
          plot_list[[23]], plot_list[[24]], plot_list[[25]], plot_list[[26]], plot_list[[28]],
          common.legend = T, legend = "right", ncol = 4, nrow = 5, align = "hv")

# Plot summarizing the p-values for all cell types ####
stats_sum$Log10 <- 0

for (i in 1:nrow(stats_sum)){
  stats_sum[i,5] <- (log10(stats_sum[i,3]))*-1
}
...
ggplot(data = stats_sum, 
  ...
  scale_y_discrete(limits = c("Tenocytes", "T cells (Cycling; Cd3e+)", "T cells (Cd4+)", "Schwann and Neural/Glial cells",
                              ...
                              "FAPs (Pro-remodeling)", "FAPs (Adipogenic)", "Endothelial cells (Vein)", 
                              "Endothelial cells (Capillary)", "Endothelial and Myeloid cells", "Dendritic cells (Xcr1+)", 
                              "Dendritic cells (Cd72+)", "Dendritic cells (Cd209a+)", "FAPs (Stem)", "Endothelial cells (Artery)",
